...
void loop()
{
    unsigned long currentTime = millis();    // loop timer counter
    
    if (currentTime - previousTime_1 >= 200) {    // loop LTM read and MAVLink data @ 5 Hz
    ...
    if (currentTime - previousTime_2 >= 1000) {    // loop MAVLink heartbeat @ 1 Hz
...
void command_heartbeat(uint8_t system_id, uint8_t component_id, uint8_t system_type, uint8_t autopilot_type, uint8_t base_mode, uint32_t custom_mode, uint8_t system_state) {
    ...
    // Send the message
    Udp.beginPacket(UDP_broadcast, remoteport);
    Udp.write(buf, len);
    Udp.endPacket();
